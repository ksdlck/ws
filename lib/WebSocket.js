...
function initAsClient(address, options) {
  ...
  if (options.value.socket) {
    // because setting the agent here conflicts with the Node v0.4.x
    // compatibility, throw an error
    if (isNodeV4)
      throw new Error('creating WebSocket with preexisting socket incompatible with Node v0.4.x');

    requestOptions.agent = {
      addRequest: function(req) {
        req.onSocket(options.value.socket);
      }
    };
  }
